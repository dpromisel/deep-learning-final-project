...
def Self_Attention(K, V, Q, use_mask=False):
	...
	prod = tf.matmul(Q, K)
	div = prod/np.sqrt(window_size_keys)
	if (use_mask == True):
		div = div + atten_mask
	sm = tf.nn.softmax(div)
...
class Atten_Head(keras.layers.Layer):
	def __init__(self, input_size, output_size, use_mask):
		...
		self.K = self.add_weight(name='K', shape=(input_size, output_size), initializer='uniform', trainable=True)
		self.V = self.add_weight(name='V', shape=(input_size, output_size), initializer='uniform', trainable=True)
		self.Q = self.add_weight(name='Q', shape=(input_size, output_size), initializer='uniform', trainable=True)
	...
	def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
		...
		self_attn = Self_Attention(K, V, Q, self.use_mask)
		return self_attn
